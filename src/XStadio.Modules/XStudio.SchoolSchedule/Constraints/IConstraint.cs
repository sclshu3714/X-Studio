...
namespace XStudio.SchoolSchedule.Constraints {
    ...
    public class IConstraint {
        public IConstraint(string id) {
            Id = id;
        }

        public IConstraint(PriorityMode priority)
            : this(Guid.NewGuid().ToString()) {
            Priority = priority;
        }

        public IConstraint(PriorityMode priority, RuleMode mode)
            : this(priority) {
            @Mode = mode;
        }

        public IConstraint(PriorityMode priority, RuleMode mode, RuleType type)
            : this(priority, mode) {
            @Type = type;
        }

        public IConstraint(PriorityMode priority, RuleMode mode, RuleType type, ActionRangeType rangeType, List<string> actionRange)
